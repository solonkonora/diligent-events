import { generateBookingReply } from "./aiEmailReply";
...
export async function bookingConfirmationEmailAI(
  clientName: string,
  eventType: string,
  eventDate: string,
  services: string[]
  ...
  // generate AI reply
  const aiReply = await generateBookingReply({
    clientName,
    eventType,
    eventDate,
  ...
  // return HTML template with AI reply included
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
      <div style="text-align: center; margin-bottom: 20px;">
        <h1 style="color: #3b82f6;">Booking Confirmation</h1>
